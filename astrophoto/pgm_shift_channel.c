// program for shifting .CR2 --> .pgm channel
//
//
...
typedef struct inputparameters
{
	...
	double shift_factor;			// all pixel data will be shifted by this factor
}inputparameters;
...
int main(int argc, char **argv)
{
	...
	ShiftData(all_px.data, all_px.width, all_px.height, all_px.pixel_size, input_args.shift_factor);
...
int CheckInputParameters(int argc, char **argv, inputparameters *input_args)
{
	if(argc == 4)
	{
		...
		input_args->shift_factor = atof(argv[3]);
		printf("Shift factor = %lf\n", input_args->shift_factor);
	}
	else
	{
		return -1;
...
{
	printf("Usage:\n");
	printf("\t%s <inputfilename> <outputfilename> [shift factor]\n", argv[0]);
